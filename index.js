...
let meilleurTime ;
var tempsPrepration;
var transitDiplayMap
var AfficheVoiture = false
...
function initMap() {
   ...
   calculateAndDisplayTrain(directionsService);
  ...
  directionsService.route(
    {
      ...
      origin: {lat: adress1Lat, lng: adress1Lng},
      destination:  {lat: adress2Lat, lng: adress2Lng},
...
// TRAIN
  function  calculateAndDisplayTrain(directionsService  ) {
      ...
      (response, status) => {
        if (status === "OK") {
         // directionsRenderer.setDirections(response);
          ...
          let infoHeuretransport = `PROCHAIN DEPART A ${heureDepartTrain} Arrivée prévu a ${arrivee}`
  ...
  //VELO
  function calculateAndDisplayBicycle3(directionsService, directionsRenderer) {
    const start = {lat: adress1Lat, lng: adress1Lng}
    const end =  {lat: adress2Lat, lng: adress2Lng}
    directionsService.route(
      {
...
//A PIED
  ...
  function calculateAndDisplayWalking5(directionsService, directionsRenderer) {
    const start = {lat: adress1Lat, lng: adress1Lng}
    const end =  {lat: adress2Lat, lng: adress2Lng}
    directionsService.route(
      {
...
console.log('nativia, ',tempsConverti)
  ...
  function recommendation(){
    ...
    var myArray = [
        {"ID": 1, "Time": infoVoiture, mode: 'Voiture' , val:'DRIVING'},
        {"ID": 2, "Time": infotrain , mode: 'TRANSPORT', val:'TRANSIT'},
        {"ID": 3, "Time": infoVelo, mode: 'VELO', val: 'BICYCLING'},
        {"ID": 4, "Time": infoPied, mode: 'A PIED', val:'WALKING'}
    ]
    ...
    for(var i=0; i<myArray.length; i++) {
        for(key in myArray[i]) {
          if(myArray[i]["Time"] === minimum) {
           ...
           let changeCarte = myArray[i]["val"]
           $("#mode").val(changeCarte).change()
           console.log(changeCarte)
           console.log( $("#mode").val())
           
           const phraseRecommande = ` Nole late vous recommende  de prendre le mode de transport : ${meilleurMode}`
